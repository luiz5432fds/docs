...
## 20. Classic Guitar Effects

Monte um conjunto de JSFX que simule pedais famosos de guitarra (overdrive, chorus, delay e wah). Cada efeito deve possuir controles simples inspirados em modelos clássicos como Tube Screamer, Boss CE-1 e Cry Baby. Encadeie esses módulos em scripts Lua para carregar rapidamente uma cadeia de pedais.

## 21. Tape Simulator

Para reproduzir a saturação de gravações em fita, desenvolva um JSFX que combine compressão suave com ruído de fita e variação sutil de velocidade (*wow & flutter*). Ajuste parâmetros como **bias** e **tape speed** para aproximar rolos lendários como o Studer A800.

## 22. Channel Strip Emulations

Crie um canal completo inspirado em mesas clássicas (Neve 1073, SSL 4000, API). O JSFX deve incluir pré‑amplificador, filtro de graves, equalizador paramétrico e compressor. Use curvas de resposta aproximadas conforme descritas em fóruns de áudio e testes de usuários.

## 23. Groove‑Based Quantizer

Desenvolva um quantizador inteligente que analisa o estilo atual e aplica "swing" baseado em performances de grandes músicos. Para sanfona, utilize como referência **Dominguinhos** e **Luiz Gonzaga**. Para percussão, modele o groove em **Naná Vasconcelos**, **Quartinha** e **Jackson do Pandeiro**. O algoritmo ajusta a posição de cada nota segundo curvas extraídas desses intérpretes, preservando a musicalidade.
